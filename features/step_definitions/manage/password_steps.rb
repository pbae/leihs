...
def fill_in_user_information(attrs)
  ...
  find(selector, match: :prefer_exact, text: _('Last name')).find('input').set attrs[:firstname] if attrs[:lastname]
  find(selector, match: :prefer_exact, text: _('First name')).find('input').set attrs[:firstname] if attrs[:firstname]
  find(selector, match: :prefer_exact, text: _('E-Mail')).find('input').set attrs[:email] if attrs[:email]
  find(selector, match: :prefer_exact, text: _('Login')).find('input').set attrs[:login] if attrs[:login]
  find(selector, match: :prefer_exact, text: _('Password')).find('input').set attrs[:password] if attrs[:password]
  find(selector, match: :prefer_exact, text: _('Password Confirmation')).find('input').set attrs[:password_confirmation] if attrs[:password_confirmation]
  click_button _('Save')
end
